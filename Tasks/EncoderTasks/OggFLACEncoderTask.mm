/*
 *  $Id: OggFLACEncoderTask.mm 1387 2009-07-17 20:18:40Z stephen_booth $
 *
...
- (void) writeTags
{
	...
	if(nil != year)
		f.tag()->addField([AudioMetadata customizeOggFLACTag:@"DATE"], TagLib::String([year UTF8String], TagLib::String::UTF8));
	...
	if(nil != trackNumber)
		f.tag()->addField([AudioMetadata customizeOggFLACTag:@"TRACKNUMBER"], TagLib::String([[trackNumber stringValue] UTF8String], TagLib::String::UTF8));
	
	// Track total
	trackTotal = [metadata trackTotal];
	if(nil != trackTotal)
		f.tag()->addField([AudioMetadata customizeOggFLACTag:@"TRACKTOTAL"], TagLib::String([[trackTotal stringValue] UTF8String], TagLib::String::UTF8));
	
	// Disc number
	discNumber = [metadata discNumber];
	if(nil != discNumber)
		f.tag()->addField([AudioMetadata customizeOggFLACTag:@"DISCNUMBER"], TagLib::String([[discNumber stringValue] UTF8String], TagLib::String::UTF8));
	
	// Discs in set
	discTotal = [metadata discTotal];
	if(nil != discTotal)
		f.tag()->addField([AudioMetadata customizeOggFLACTag:@"DISCTOTAL"], TagLib::String([[discTotal stringValue] UTF8String], TagLib::String::UTF8));
	
	// Compilation
	compilation = [metadata compilation];
	if(nil != compilation)
		f.tag()->addField([AudioMetadata customizeOggFLACTag:@"COMPILATION"], TagLib::String([[compilation stringValue] UTF8String], TagLib::String::UTF8));
